...
class FslSvm:
    def __init__(self, training=None, testing=None):
        self.raw_train_data = None
        self.raw_test_data = None

        # ONLY load CSV if path is provided
    ...
    def predict_frame(self, frame_bytes:str=None, img_path:str=None):
        """
        frame_bytes = base64 encoded image from frontend
        """
        ...
        features = self._processed_data(closing)

        # predict
        pred = self.svm.predict([features])[0]

        conf = max(self.svm.predict_proba([features])[0])
    ...
    def load_svm_model(self, svm_model_path="/home/code871/Git/fsl-svm/server2/svm_hog_model.pkl"):
        self.svm = joblib.load(svm_model_path)
        print("SVM model loaded.")

    def save_svm_model(self, svm_model_path="/home/code871/Git/fsl-svm/server2/svm_hog_model.pkl"):
